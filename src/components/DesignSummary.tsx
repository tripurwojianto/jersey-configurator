...
export const DesignSummary: React.FC<DesignSummaryProps> = ({
  ...
  const getWhatsAppUrl = () => {
    ...
    const playerName = config.playerName || 'SYAMANAH';
    ...
    const lines = [
      `Halo Admin Syamanah Garmen, saya ingin melakukan pemesanan custom jersey:`,
      ``,
      `*DATA PESANAN:*`,
      `• Model: ${model}`,
      `• Pattern: ${pattern}`,
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 sm:px-6 py-4 border-b border-slate-200 bg-white shrink-0">
          <div className="flex items-center gap-3">
            <div className="p-2 rounded-lg bg-teal-50 border border-teal-200 text-[#0D9488]">
              <ShieldCheck className="w-5 h-5" />
            </div>
            <div>
              <h2 className="text-base font-bold text-slate-900 flex items-center gap-2">
                Custom Apparel Design Specification
                <span className="text-[11px] font-mono bg-teal-50 text-[#0D9488] px-2 py-0.5 rounded border border-teal-200 font-semibold">
                  MVP Ready
                </span>
              </h2>
              <p className="text-xs text-slate-500">
                Customer approval record & serialized JSON parameters
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-4 sm:p-6 overflow-y-auto space-y-5 sm:space-y-6 flex-1">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 p-3.5 rounded-xl bg-teal-50/60 border border-teal-200/80 text-xs text-slate-700">
            <div>
              <p className="font-bold text-[#0B2F64]">
                Production Hand-off Architecture Ready
              </p>
              <p className="text-[11px] text-slate-600 mt-0.5">
                This JSON payload contains all parameters required by dye-sublimation cutting plotters and production ERPs.
              </p>
            </div>
            <button
              id="btn-simulate-api"
              ...
              className="flex items-center gap-2 px-4 py-2 bg-[#0B2F64] hover:bg-[#082247] text-white font-semibold rounded-lg text-xs transition-all shadow-sm active:scale-95 disabled:opacity-50 whitespace-nowrap cursor-pointer"
            >
              {isSimulatingApi ? (
                <>
        ...
        <div className="flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-end gap-2.5 sm:gap-3 px-5 sm:px-6 py-3.5 sm:py-4 border-t border-slate-200 bg-slate-50 shrink-0">
          ...
          <a
            ...
            className="inline-flex items-center justify-center gap-2 px-5 py-2.5 text-xs font-bold text-white bg-[#0D9488] hover:bg-[#0f766e] active:scale-[0.98] rounded-lg transition-all shadow-sm text-center"
          >
