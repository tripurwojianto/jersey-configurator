...
export const DesignSummary: React.FC<DesignSummaryProps> = ({
  ...
  const getWhatsAppUrl = () => {
    const model = `${config.productName || 'Sujaya SJ-01'} (ID: ${config.productId || 'sujaya-sj-01'})`;
    const pattern = (config.pattern || 'DIAGONAL').toUpperCase();
    const playerName = config.playerName || '(Polos)';
    const playerNumber = config.playerNumber ? `#${config.playerNumber}` : '(Tanpa Nomor)';
        ...
        <div className="p-4 sm:p-6 overflow-y-auto space-y-5 sm:space-y-6 flex-1">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-3">
              ...
              <span className="text-[10px] text-indigo-600 font-mono font-medium">{config.productId}</span>
            </div>
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-3">
              ...
              <p className="text-sm font-bold text-slate-900 mt-1 truncate">
                {config.playerName || '(Polos / Tanpa Nama)'}
              </p>
              <div className="flex items-center gap-2 mt-0.5">
                <span className="text-xs font-mono font-bold text-[#0B2F64]">
                  {config.playerNumber ? `#${config.playerNumber}` : '(Tanpa Nomor)'}
                </span>
