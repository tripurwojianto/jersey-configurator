...
export class JerseyModel {
  ...
  public async loadExternalGlb(url: string, config: JerseyConfig): Promise<void> {
    ...
    try {
      ...
      // Auto-center and normalize scale to 2.4 height precisely at origin (0, 0, 0)
      ...
      if (maxDim > 0) {
        ...
        // Correct position offset after scaling so the true geometric center is (0, 0, 0)
        this.gltfGroup.position.copy(center).multiplyScalar(-targetScale);
      } else {
        this.gltfGroup.position.sub(center);
      }
      this.gltfGroup.updateMatrixWorld(true);
      ...
      this.gltfGroup.traverse((node) => {
        if (node instanceof THREE.Mesh) {
          ...
          const nName = `${node.name || ''} ${mat?.name || ''}`.toLowerCase();
          if (
      ...
      const isSujaya =
        ...
        url.includes('jersey_revisi');

      this.modelMetadata = {
        isSeparatedMaterials: !isSujaya && (hasSeparatedParts || matNames.length > 1),
  ...
  private applyGltfMaterials(config: JerseyConfig) {
    ...
    const isSujaya =
      config.productId === 'sujaya-sj-01' ||
      config.modelId === 'sujaya-sj-01' ||
      (this.currentGlbUrl && this.currentGlbUrl.includes('jersey_revisi'));
    ...
    this.gltfGroup.traverse((node) => {
      if (node instanceof THREE.Mesh) {
        ...
        if (this.modelMetadata.isSeparatedMaterials && !isSujaya) {
          ...
          // Single unified mesh model (e.g., athletic_jersey.glb / sujaya-sj-01 / pro_jersey.glb)
          ...
          const assignUnified = (m: THREE.Material): THREE.Material => {
            if (m instanceof THREE.MeshStandardMaterial) {
              m.map = unifiedTex;
              m.color.set(0xffffff); // Pure white so canvas print colors render true
              m.roughness = 0.65;
              m.metalness = 0.05;
              ...
              const newMat = new THREE.MeshStandardMaterial({
                ...
                roughness: 0.65,
                metalness: 0.05,
                normalMap: (m as any).normalMap || null,
                normalScale: (m as any).normalScale || new THREE.Vector2(1.5, 1.5),
                aoMap: (m as any).aoMap || null,
          ...
          if (Array.isArray(node.material)) {
            node.material = node.material.map(assignUnified);
          } else if (node.material) {
            node.material = assignUnified(node.material);
          }
        }
      }
    });
