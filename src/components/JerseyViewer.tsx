...
export const JerseyViewer = forwardRef<JerseyViewerHandle, JerseyViewerProps>(({
  ...
  useEffect(() => {
    ...
    // 4. OrbitControls with Rotate, Zoom, and Pan
    const controls = new OrbitControls(camera, renderer.domElement);
    controls.enableDamping = true;
    controls.dampingFactor = 0.06;
    controls.minDistance = 1.8;
    controls.maxDistance = 8.5;
    controls.maxPolarAngle = Math.PI / 1.7; // prevent going under floor
   ...
   * (colors, patterns, player name 'Syamanah', number '10', and badge) baked permanently into a .GLB file.
   */
  const exportCurrentJerseyGLB = useCallback(async () => {
    ...
    try {
      ...
      // Safe filename: defaults to syamanah-custom-jersey.glb
      const safeName = (config.playerName || 'syamanah')
        .toLowerCase()
        .trim()
        .replace(/[^a-z0-9_-]/gi, '-');
      const filename = `${safeName || 'syamanah'}-custom-jersey.glb`;
  ...
  return (
    ...
    >
      ...
      {/* Bottom Center: Interaction HUD with Rotate, Zoom, Pan hints */}
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 pointer-events-none hidden md:flex items-center gap-3 bg-white/90 backdrop-blur-md border border-slate-200 shadow-sm rounded-full px-4 py-1.5 text-[11px] text-slate-500 z-10">
        ...
        <span className="flex items-center gap-1.5 font-medium">
          ...
          <span>Right-click or toggle Pan</span>
        </span>
      </div>
      ...
      {/* Floating GLB Export Button (Direct Download from 3D Viewport) */}
