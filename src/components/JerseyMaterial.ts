...
export class JerseyMaterialManager {
  ...
  public renderUnifiedCanvas(config: JerseyConfig): void {
    ...
    // 4. BACK OF JERSEY: Player Name & Player Number
    // UV Mapping for athletic_jersey.glb: Back Center is at U ≈ 0.758
    // Upper Back (Name): V ≈ 0.145-0.155 (with flipY = false)
    // Mid Back (Number): V ≈ 0.352 (with flipY = false, lowered for breathing room)
    const backCenterX = w * 0.758;
    const nameStyle = config.backNameStyle || 'arched';

    // 4a. Player Name
    const playerName = (config.playerName || 'Syamanah').toUpperCase().trim();
    if (playerName.length > 0) {
      // Responsive font size for 2048px canvas
      const nameFontSize = Math.round(h * (playerName.length > 10 ? 0.038 : 0.042)); // ~78-86px
      ctx.font = `bold ${nameFontSize}px '${fontName}', 'Chakra Petch', sans-serif`;

      // Dark athletic outline for legibility on all garment colors
      const strokeWidth = Math.max(3, Math.round(nameFontSize * 0.08));
      const strokeColor = 'rgba(0, 0, 0, 0.45)';

      if (nameStyle === 'arched') {
        // Arched name following upper back / shoulder curvature
        const apexY = h * 0.145;
        const textLen = ctx.measureText(playerName).width;
        const arcRadius = Math.max(textLen * 1.35, h * 0.28);
    ...
    // Scaled up ~23% (from 0.145 to 0.178) for dominant presence and shifted down to V ≈ 0.352
    const playerNumber = (config.playerNumber || '10').trim();
    if (playerNumber.length > 0) {
      ctx.save();
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';

      const numFontSize = Math.round(h * 0.178); // ~365px dominant athletic number
      ctx.font = `800 ${numFontSize}px '${fontName}', 'Chakra Petch', sans-serif`;

      const numY = h * 0.352; // Lowered from 0.325 for balanced spacing with name and bottom hem

      // Outer contrasting stroke for professional athletic jersey appearance
      ctx.lineWidth = Math.max(7, Math.round(numFontSize * 0.075));
      ctx.strokeStyle = 'rgba(0, 0, 0, 0.55)';
      ctx.strokeText(playerNumber, backCenterX, numY);

      // Main Number Ink Fill
      ctx.fillStyle = numberInkColor;
      ctx.fillText(playerNumber, backCenterX, numY);
      ctx.restore();
    }
  ...
  private renderBackCanvas(config: JerseyConfig) {
    ...
    drawBackTextOverlay(ctx, w, h, {
      name: config.playerName || 'Syamanah',
      number: config.playerNumber || '10',
